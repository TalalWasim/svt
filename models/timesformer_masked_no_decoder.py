...
class MaskedVisionTransformerEncoder(nn.Module):
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_token', 'time_embed', 'mask_token'}
...
class MaskedVisionTransformerNoDecoder(nn.Module):
    ...
    def forward(self, x, mask=False):
        ...
        x_cls, x_mask_pred, x_mask_lab, masks, W, H, T = self.encoder(x, mask=mask)
...
def get_masked_vit_base_patch16_224_no_decoder(cfg, no_head=False, **kwargs):
    patch_size = 16
    vit = MaskedVisionTransformerNoDecoder(img_size=cfg.DATA.TRAIN_CROP_SIZE,
                                   patch_size=patch_size,
