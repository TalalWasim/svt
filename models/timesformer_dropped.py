...
def getClosestFactors(inp):
    sq = math.floor(math.sqrt(inp))
    while(inp%sq != 0):
        sq -= 1
    return sq
...
class DroppedVisionTransformerEncoder(nn.Module):
    """Dropped Vision Transformer"""
    ...
    def forward_features(self, x, get_all=False, get_attn=False, mask=False):
        ...
        if mask:
            ...
            x = x[~masks].reshape(B, -1, C)
            
            W = getClosestFactors(x.shape[1]//T)
            x = torch.cat((cls_tokens, x), dim=1)
        ...
        x = self.norm(x)
        if mask:
            return (x[:,0], x[:,1:,:], x_mask_lab, masks, W_old, H_old, T)
        if get_all:
            return x
        return x[:, 0]
...
class DroppedVisionTransformerDecoder(nn.Module):
    """ MAE Decoder
    """
    ...
    def forward(self, x, masks, W, H, T, get_masked=True):
        ...
        # x[B, 320, 512]        
        B, num_vis_tokens, C = x.shape
        num_mask_tokens = W*H*T - num_vis_tokens
        
        out = torch.zeros(B, W*H*T, C)
        out = out.to(x.device)
        x = rearrange(x, 'b (n t) m -> (b n t) m', b=B, t=T)
        out[~masks] = x
        out[masks] += self.mask_token.squeeze(0)
        
        x = rearrange(out, 'b (n t) m -> (b t) n m', b=B, t=T)
...
class DroppedVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = DroppedVisionTransformerEncoder(
            img_size=img_size,
        ...
        self.decoder = DroppedVisionTransformerDecoder(
            patch_size=patch_size, 
